...
async function forward(request: NextRequest, method: 'GET' | 'POST' | 'PATCH' | 'PUT' | 'DELETE') {
  ...
  const suffix = request.nextUrl.pathname.replace(/^\/api\/playlists/, '');
  ...
  for (const basePath of PLAYLIST_PATH_CANDIDATES) {
    const url = `${API_BASE}${basePath}${suffix}${query ? `?${query}` : ''}`;
      ...
      if (res.ok && text && (method === 'GET' || method === 'POST' || method === 'PATCH')) {
        try {
          ...
          if (Array.isArray(data)) {
            console.log('[API playlists/...path] Response is array with', data.length, 'items');
          }
          // Log if it's a single playlist
          else if (data.id && data.name) {
            console.log('[API playlists/...path] Response is single playlist:', data.id);
          }
        } catch (error) {
          console.warn('[API playlists/...path] Response parse warning:', error);
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Proxy playlists GET catch-all error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Proxy playlists POST catch-all error:', error);
    return NextResponse.json({ error: 'Erreur du serveur proxy playlists' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error('Proxy playlists PUT catch-all error:', error);
    return NextResponse.json({ error: 'Erreur du serveur proxy playlists' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    console.error('Proxy playlists PATCH catch-all error:', error);
    return NextResponse.json({ error: 'Erreur du serveur proxy playlists' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Proxy playlists DELETE catch-all error:', error);
    return NextResponse.json({ error: 'Erreur du serveur proxy playlists' }, { status: 500 });
  }
}
